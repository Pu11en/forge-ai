...
// FIX: Add PILLARS constant from the main app, as it's needed for context.
...
// BEST PRACTICE: Use Firebase Functions config for secrets.
// To set this value, run the following command in your terminal:
// firebase functions:config:set gemini.key="YOUR_GEMINI_API_KEY"
const geminiApiKey = functions.config().gemini?.key;
if (!geminiApiKey) {
    console.error("Gemini API key not found in Firebase Functions config. Please set it using the Firebase CLI.");
}
const ai = new GoogleGenAI({apiKey: geminiApiKey});


export const generateSoulPrint = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
     if (!geminiApiKey) {
        throw new functions.https.HttpsError("internal", "Gemini API key is not configured on the server.");
    }

    const {answers} = data;
    if (!answers) {
        throw new functions.https.HttpsError("invalid-argument", "The function must be called with an 'answers' argument.");
    }
...
Your goal is to create a seamless, resonant experience where the user feels like they are interacting with a version of themselves.`;
        ...
        return {pillarSummaries, systemPrompt: systemPromptInstruction};
    } catch (error) {
        console.error("Error generating SoulPrint:", error);
        throw new functions.https.HttpsError("internal", "Failed to generate SoulPrint.", error);
    }
});


export const continueChat = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
    }
     if (!geminiApiKey) {
    ...
    if (!messages || !systemPrompt) {
        throw new functions.https.HttpsError("invalid-argument", "The function requires 'messages' and 'systemPrompt'.");
    }
    ...
    try {
        ...
        throw new functions.https.HttpsError("internal", "Failed to get AI response.", error);
    }
});